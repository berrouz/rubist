...
class Array
  def	find_two_missed
    ...
    return temp.first.return_which_is_missed if temp.size ==1
  ...
  def shorten
    case get_shift
    when 0
      [self[mid_position,size-mid_position]]
    when 1
      [self[0, mid_position+1], self[mid_position,size-mid_position+1]]
    when 2
  ...
  def get_shift
    ...
    when mid_position+first
      0
    when mid_position+1+first
      1
    when mid_position+2+first
      2
    end
  end

  def return_which_is_missed
    (first+1..last-1).to_a
  end
...
describe Array do
	...
	it "should return missed from array" do
		[1,3].return_which_is_missed.should == [2]
	end
